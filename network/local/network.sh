...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating motorvehicledepartment Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-motorvehicledepartment.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating licinsurance Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-licinsurance.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating bajajinsurance Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-bajajinsurance.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating benzmanufacturer Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-benzmanufacturer.yaml --output="organizations"
    res=$?
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/motorvehicledepartment/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    createbajajinsurance

    infoln "Creating benzmanufacturer Identities"

    createbenzmanufacturer
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    if [ "$OS_ARCH" == "darwin-amd64" ]; then
      ...
      rm -rf organizations/fabric-ca/motorvehicledepartment
      rm -rf organizations/fabric-ca/licinsurance
      rm -rf organizations/fabric-ca/bajajinsurance
      rm -rf organizations/fabric-ca/benzmanufacturer
      ...
      ## remove fabric ca artifacts
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/motorvehicledepartment'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/licinsurance'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/bajajinsurance'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/benzmanufacturer'
